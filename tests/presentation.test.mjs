...
test('entry view splits into expenses and income sections with deep links',async()=>{
  ...
  assert.deepEqual(routes.map(([, ,route])=>route),['overview','expenses','entries']);
  assert.match(html,/<a href="#overview"[^>]*data-route="overview"/);
  assert.match(html,/<a href="#expenses"[^>]*data-route="expenses"/);
  assert.match(html,/<a href="#entries"[^>]*data-route="entries"/);
  assert.match(html,/data-entry-section/);
  assert.match(html,/data-exp-tab/);
...
test('expense entry is compact, title-first, category-aware and guarded against double submit',async()=>{
  ...
  assert.match(app,/setTimeout\(\(\) => \$\('expense-amount'\)\.focus\(\), 0\)/);
  assert.match(css,/\.expense-form-grid \{ display: grid; grid-template-columns: 1fr 1fr;/);
  assert.match(css,/#expense-fields \{ grid-template-columns: minmax\(0, 1fr\) minmax\(0, 1\.1fr\);/);
...
test('expense rows and transfer form keep a usable mobile layout',async()=>{
  ...
  assert.match(html,/id="transfer-swap"[^>]*aria-label="Поменять местами счета и суммы"/);
  assert.match(app,/\[from\.value, to\.value\] = \[to\.value, from\.value\]/);
  assert.match(app,/\[fromAmount\.value, toAmount\.value\] = \[toAmount\.value, fromAmount\.value\]/);
  assert.match(css,/\.transfer-route \{[^}]*background: transparent;/);
