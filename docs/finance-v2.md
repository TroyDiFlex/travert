...
Статус на 2026-09-14: реализованы деньги, даты, счета, категории, расходы, локальная проекция, очередь и атомарное сохранение через IndexedDB. Чистые проверки проходят; проверка повторного открытия реальной IndexedDB подготовлена, но ещё не запускалась в браузере. Firebase, синхронизация и новый интерфейс пока не реализованы. Технический контракт вынесен в [синхронизацию](finance-v2-sync.md).
