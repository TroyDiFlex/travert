...
| :--- | :--- |
| Серверное хранилище | Не выбрано. Обязательное условие — работа без карты и платёжного профиля. До выбора продукт полноценно работает локально. |
| Учётные записи | Локальный профиль устройства на первом этапе; удалённая идентичность появится вместе с допустимым сервером. |
...
| :--- | :--- |
| Firestore + Firebase Auth | Исключён: пользователь столкнулся с требованием привязать карту. Платёжный профиль для этого проекта не создаётся. |
...
Выбор сервера отложен и не блокирует продукт: интерфейс читает и сохраняет данные через IndexedDB. Сервер обязан поддержать отдельные пользователи, условную запись по версии, неизменяемый `opId`, журнал `seq` и квитанции либо эквивалентный атомарный контракт. Перед выбором отдельно проверяются регистрация без карты, бесплатные лимиты, экспорт данных и возможность смены поставщика.
...
flowchart LR
  ...
  SYNC <--> REMOTE["Сменный сервер: данные, журнал, квитанции"]
  AUTH["Сменная идентичность"] --> SYNC
...
Локальная база обслуживает интерфейс; после выбора сервера его подтверждённая версия станет общей точкой согласования устройств. Очередь содержит ещё не подтверждённые намерения пользователя. Балансы и отчёты вычисляются из записей, включая явно отмеченные локальные изменения.
...
Сначала один локальный профиль владеет одной личной книгой. Локальная база разделяется по пространству сервера, идентификатору пользователя и книге; до подключения сервера используются стабильные локальные значения. После появления удалённого входа два пользователя не должны читать или менять книги друг друга. Совместные семейные книги, роли и приглашения — отдельное расширение.

Первый удалённый вход потребует сети. После успешной настройки на личном устройстве уже сохранённая книга открывается без сети. При смене пользователя предыдущая очередь не отправляется от имени нового пользователя. Истёкшая авторизация приостанавливает синхронизацию, но не удаляет изменения.
...
Автоматическую двустороннюю запись между Таблицей и новым сервером не проектировать. До перехода возврат — использование прежнего сайта. После появления уникальных расходов в v2 старая версия не сможет их показать: перед возвратом сохранить полный экспорт v2, а не пытаться втиснуть расходы в формат доходов.
...
| 2. Серверная синхронизация | Адаптер допустимого сервера, изоляция, транзакции, журнал, квитанции и конфликты в локальном тестовом окружении. | Два пользователя изолированы; два устройства не теряют записи; потерянный ответ не создаёт дубль. |
...
Предлагаемые модули: `finance/core/`, `finance/repository.js`, `finance/local/`, `finance/sync/`, `finance/remote/`, `finance/auth/`, `finance/migrations/`. Использовать нативные ES-модули и Web API; сторонние UI-фреймворки и вспомогательные runtime-библиотеки не добавлять. Любой будущий сервер изолируется в `remote` и `auth`, не импортируется финансовым ядром или UI и не требует карты. Иконки входят в статические ресурсы для офлайн-запуска.

До создания облачной базы остаются проверки: регистрация без платёжного профиля, выполнимость транзакционного протокола, изоляция пользователей, экспорт и независимые резервные копии. Они не мешают разработке локального продукта.
