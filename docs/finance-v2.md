...
Статус: проект архитектуры от 2026-09-14. Финансовое ядро, IndexedDB, Firebase и новый интерфейс пока не реализованы. Технический контракт вынесен в [синхронизацию](finance-v2-sync.md).
...
| Публикация | В будущем GitHub / GitHub Pages по отдельному запросу. Сейчас только локальная работа. |
...
Предлагаемый источник иконок — локально поставляемая библиотека [Lucide](https://lucide.dev/), с сохранением её лицензии. Начальный каталог выбора — минимум 150 подходящих иконок: еда, дом, покупки, транспорт, здоровье, образование, работа, путешествия, развлечения и финансы. Это целевой объём реализации, пока библиотека не добавлена.
...
Предлагаемые модули: `finance/core/`, `finance/repository.js`, `finance/local/`, `finance/sync/`, `finance/remote/firestore.js`, `finance/auth/`, `finance/migrations/`. Использовать существующие ES-модули; точные зависимости фиксировать при реализации. SDK и иконки должны поставляться с приложением для офлайн-запуска.
